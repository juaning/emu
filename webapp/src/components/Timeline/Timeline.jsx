...
function Timeline({ ...props }) {
  ...
  const timelineClass =
    `${classes.timeline} ${cx({ [classes.timelineSimple]: simple })}`;
  return (
    <ul className={timelineClass}>
      {stories.map((prop) => {
        const storyKey = shortid.generate();
        const panelClasses =
          `${classes.timelinePanel} ${cx({
            [classes.timelinePanelInverted]: prop.inverted,
            [classes.timelineSimplePanel]: simple,
          })}`;
        const timelineBadgeClasses =
          `${classes.timelineBadge} ${classes[prop.badgeColor]} ${cx({
            [classes.timelineSimpleBadge]: simple,
          })}`;
        return (
          <li className={classes.item} key={storyKey}>
            {prop.badgeIcon ? (
